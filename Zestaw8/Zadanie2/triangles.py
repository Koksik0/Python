...
class Triangle:
    ...
    @classmethod
    def from_points(cls, list_of_points: list[Point]):
        if list_of_points[0].x * (list_of_points[1].y - list_of_points[2].y) + list_of_points[1].x * (
                list_of_points[2].y - list_of_points[0].y) + list_of_points[2].x * (
                list_of_points[0].y - list_of_points[1].y) == 0:
            raise ValueError("Punkty nie powinny leżeć na jednej prostej")
        return cls(list_of_points[0].x, list_of_points[0].y, list_of_points[1].x, list_of_points[1].y,
